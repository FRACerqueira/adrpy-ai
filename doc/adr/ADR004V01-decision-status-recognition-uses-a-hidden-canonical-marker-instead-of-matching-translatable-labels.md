...
Both are byte-for-byte replicas of the real AdrPlus (C#) tool's own design (confirmed directly in its source, in the AdrPlus repository: status matching in `AdrPlusRepoConfig.StatusMapping`/`Helper.ParseStatusLine`, `src/AdrPlus/Domain/AdrPlusRepoConfig.cs:179-186` and `Core\Helper.cs:253-283`; separator matching in `AdrService.cs:575`, `supersedeParts[0].IndexOf(separator, ...)`) -- neither is a defect introduced by this port; AdrPlus has the identical fragility in both cases.
