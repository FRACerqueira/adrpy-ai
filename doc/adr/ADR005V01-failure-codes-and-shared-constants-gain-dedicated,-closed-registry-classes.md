...
* The same class of doc-drift (a failure code that exists in code but not in `describe()`) has now been found twice in the same audit rounds, at increasing scope each time -- a purely reactive, read-the-source-and-patch-it-by-hand fix doesn't change the odds of a third recurrence.
...
* Related: the broader security-audit rounds of the same period (`2026-09-21` decision-log entries, scope `security`), during which the second, larger wave of undocumented codes was found.
