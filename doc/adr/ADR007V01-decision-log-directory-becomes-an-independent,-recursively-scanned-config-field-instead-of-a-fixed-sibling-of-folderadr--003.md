...
* Related: [ADR005V01](ADR005V01-failure-codes-and-shared-constants-gain-dedicated,-closed-registry-classes.md) -- the same "recognize the constraint is broader than first scoped" reasoning; its registry refactor, deferred when this ADR was accepted, was implemented later the same day (`2026-09-21--scope-note--core--adr005-failure-codes-and-constants-registry-implemented.md`).
...
* Revised (round 31): the real-filesystem-resolution guard added in the round 30 revision was itself found to have a check-then-use TOCTOU window, confirmed live and reliably winnable -- see the round 31 decision-log entry closing that finding. The second check that narrowed it was removed with the lock in the architectural review (Round 43); see part 2 of the Decision Outcome.
