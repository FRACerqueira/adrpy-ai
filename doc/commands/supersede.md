...
| `filename-too-long` | The successor's own title makes a file name longer than the filesystem allows once the temp file's suffix is added (data.filename) -- nothing was written; shorten --title. |
...
| `supersede-successor-write-failed` | Preparing either file, or creating the successor (the FIRST of the two commits), failed -- no decision was changed (data.intended_successor names the file that would have been created). If the empty reservation of that name could not be removed, it is left as a 0-byte file that check names: remove it. |
