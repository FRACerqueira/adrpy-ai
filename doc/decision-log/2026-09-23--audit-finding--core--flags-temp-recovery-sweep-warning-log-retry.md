...
Architectural review (Round 43): the atomic_write item was already retracted (`2026-09-23--retraction--core--swept-temp-retry-reverted.md`). The doc/skills/README.md item is now the general rule: adrpy-skills, like adrpy, runs one command at a time on a working copy (ADR001). The parse_flags, orphan-sweep and log items stand.
