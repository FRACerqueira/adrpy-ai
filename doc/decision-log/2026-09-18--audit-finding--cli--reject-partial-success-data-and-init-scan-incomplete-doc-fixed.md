...
Architectural review (Round 43): Finding 1's premise is gone: reject reads its predecessor's family from the snapshot validated before any write, so no family scan runs after its first write, and `family-scan-incomplete` was replaced by the validator's single `scan-incomplete` error, reported before anything is written. Finding 2 (`init-existing-numbers-scan-incomplete` documented for every init path) stands.
