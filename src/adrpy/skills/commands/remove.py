...
def describe():
    return {
        ...
        "description": (
            ...
            "unless --force is given -- reported in `skipped`, not as a failure. So is an agentsmd "
            "block indented four spaces or more (or by a tab) whose marker hash still matches, with reason "
            "'indented': it is byte for byte what a verbatim copy you quoted would be, so deleting it "
            "takes --force; any other indented copy (no valid marker, edited, or repeated/unpaired "
            "tags) is your own text and never touched, even with --force. Asking to remove "
