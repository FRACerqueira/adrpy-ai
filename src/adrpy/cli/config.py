...
def describe():
    return {
        ...
        "description": (
            ...
            "(data.changed_fields names every guarded field this call touched, data.existing_decisions the "
            "total decision count found) rather than silently breaking recognition of those decisions. "
            "Status labels block if ANY recognized decision exists (current-scheme or legacy-scheme -- the "
            "header format is identical either way); --separator blocks only if a CURRENT-scheme decision "
            "exists (it has no bearing on legacy-scheme filenames); --migrationpattern blocks only if a "
            "LEGACY-scheme decision exists (it has no bearing on current-scheme filenames) -- for both "
            "--separator and --migrationpattern this is a PERMANENT block once the scheme it governs has any "
            "decision, with no migration path. Same scan-incomplete fail-closed shape as folderadr's own "
            "guard: status-or-separator-change-scan-incomplete (not scheme-scoped -- an unreadable "
            "subdirectory's own contents can't be ruled out for any guarded field). "
