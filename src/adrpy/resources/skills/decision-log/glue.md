...
`doc/decision-log/`: the lighter-weight sibling of `doc/adr/`, for events
...
`doc/decision-log/INDEX.md`'s own header -- this page does not repeat it,
...
- **Yes** -- this is not a decision-log entry. Open (or extend) an ADR
  instead, under this project's normal ADR-approval gate.
...
in `doc/decision-log/INDEX.md`'s own header; this table is the quick
lookup.
...
change to adrpy itself, not to this project.

## Step 3: write and register the entry

Once steps 1-2 are settled -- this genuinely belongs in the log, and you
know its classification -- run `adrpy log`. It owns everything mechanical
in one call: constructing the filename, formatting the
classification-specific structured line, writing the entry, and
regenerating `INDEX.md` -- and refuses outright
...
(or omitting one it requires) is a `usage-error` -- run `adrpy help log`
...
- **Cycles** -- a human-friendly name for a range of related entries,
  ...
  separately in the folder's `CYCLES.md`, if present, never as a field
